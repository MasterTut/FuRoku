...
class MenuManager:
    ...
    def import_apps(self, menu_name, apps_list):
        ...
        new_menu = Menu(Canvas.get_width() * .19, 20, Canvas.get_width(), Canvas.get_height() -40, menu_name)
        new_menu.is_list = False
        new_menu.background = False
        padding = 31
        button_width = 256
        button_height = 256
        max_width = new_menu.surface.get_width() - (padding + button_width)
    ...
    def _display_menus(self):
        ...
        for submenu in self._all_menus[self.side_menu]:
            if self._active_button.name == submenu.name or submenu == self._active_menu:
                    submenu.display()
...
class Manager:
    ...
    def move(self, key) -> None:
        ...
        if key == pygame.K_RIGHT:
            if self.menu_mgr._active_menu.is_list:
                self._switch_menu()
            else:
    ...
    def _move_in_grid(self, direction: Direction ) -> None:
        ...
        if direction == Direction.UP:
            ...
            if self.menu_mgr._active_button:
                self.menu_mgr._active_button.is_active = False
            if self.col + 1 < len(self.menu_mgr._active_menu.button_matrix[self.row]):
                self.col += 1
        ...
        if len(self.menu_mgr._active_menu.button_matrix[self.row]) > self.col:
            self.menu_mgr._active_button = self.menu_mgr._active_menu.button_matrix[self.row][self.col]
        else:
            self.menu_mgr._active_button = self.menu_mgr._active_menu.button_matrix[self.row][-1]
        self.menu_mgr._active_button.is_active = True
        self._deselect_buttons(self.menu_mgr._active_menu)
